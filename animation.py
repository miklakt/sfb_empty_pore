...
def animate_factory(
    ...
    def animate(i):
        ...
        init = False
        fig.savefig(f"frames/chi_PS_{datum.chi_PS}/{datum.pc}.pdf")
    return animate
#%%
chi_ps = 0.6
chi_pc = -1.0
...
empty_pore = get_by_kwargs(empty_pore, chi_PS = chi_ps, s = s, r = r).squeeze()
phi_center = empty_pore.phi[0]
...
z = range(len(fe))
brush = ascf_pb.BrushInsertionFreeEnergy(external_phi=phi_center)
#z = np.arange(ph, len(phi_center)-ph, 0.2)
#fe = brush.total_free_energy(chi_ps, ph, pw, pc_list, chi_pc, (a1,a2))
...
selected_data.sort_values(by = "pc", inplace = True)
selected_data.reset_index(drop = True, inplace = True)
load_datasets(selected_data, keys = ["phi"])
selected_data["delta_phi"] = selected_data.apply(lambda _: phi_change_vicinity(_.phi, empty_pore.phi, _.pc, viewport=(10,10), ph=_.ph, pw=_.pw), axis =1)
# %%
fig, axs = plt.subplots(
    nrows=2, sharex=True,
    #gridspec_kw = dict(width_ratios = (1,), height_ratios = (1.6,1))
    )
parameters_title = "$\chi_{PS} = $" + f"{chi_ps}, " +\
    "$\chi_{PC} = $" + f"{chi_pc}, " +\
    "$h = $" + f"{ph}, " +\
    "$r_{pore}$ = " + f"{r}"
#%%
animate = animate_factory(
    fig,
    axs,
    selected_data,
    z = z, fe = fe,
    title = "Nanocolloid in a cylindrical pore\n" + parameters_title,
...
#fig
#%%
ani = animation.FuncAnimation(fig, animate, frames = range(len(selected_data)))
