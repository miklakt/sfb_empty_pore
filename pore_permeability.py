...
def read_fields(a0, a1, chi_PC, chi, wall_thickness, pore_radius, dummy = False, d = None):
...
@lru_cache
def pore_permeability(a0, a1, d, chi_PC, chi, wall_thickness, pore_radius, l, dummy = False):
    fields = read_fields(a0, a1, d, chi_PC, chi, wall_thickness, pore_radius, dummy)
    conductivity = fields["conductivity"]
    empty_pore_conductivity = read_fields(a0, a1, d, chi_PC, chi, wall_thickness, pore_radius, dummy=True)["local_conductivity"]
    l1 = fields["l1"]
...
@lru_cache
def pore_conductivity_z(a0, a1, d, chi_PC, chi, wall_thickness, pore_radius, l, dummy = False):
    fields = read_fields(a0, a1, d, chi_PC, chi, wall_thickness, pore_radius, dummy)
    conductivity = fields["conductivity"]
    conductivity = conductivity[l1-l:l1+wall_thickness+l, 0:pore_radius]
    pore_radius = fields["r"]
    r = np.arange(0, pore_radius)
    P_z = np.sum(conductivity*(r+1/2), axis = 1)
    return P_z
...
L = wall_thickness
P = [[[
    pore_permeability(a0, a1, d_, chi_pc_, chi_ps_, wall_thickness, pore_radius, L)
    for d_ in D
    ]
    for chi_pc_ in CHI_PC
    ]
