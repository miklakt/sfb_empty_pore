...
# %%
def convolve_particle_volume(array, particle_d, mode = "roll"):
...
def convolve_particle_surface(array, particle_d, mode = "roll"):
    nr, nz = np.shape(array)
    particle_r = int(particle_d/2)
    array = np.pad(array, ((0, particle_r),(particle_r,particle_r)), 'edge')
    new_arr = []
...
if __name__ == "__main__":
    ...
    phi_conv = convolve_particle_volume(phi, 30, "trim")
    def mirror(arr):
        cut = arr
