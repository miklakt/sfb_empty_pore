...
plt.tight_layout()
plt.savefig(f"free_energy_{w}.pdf")
# %%
chi_PCs = np.linspace(0.5, -3.0)
chi_PSs = [0, 0.5, 0.6, 0.8]
fig, ax = plt.subplots()
for chi_ps in chi_PSs:
    D_eff_chi = [D_effective_membrane(N=N, sigma=sigma, a1=a1, a2=a2, chi_PS=chi_ps, chi_PC=chi_pc, w=w) for chi_pc in chi_PCs]
