...
\begin{figure}
    ...
    \caption{
        ...
        to the inner surface of a cylindrical pore in an impermeable membrane. The pore radius is $r_{\text{p}}$ and the thickness of the membrane is $L$.
...
Pores with such features can also be realized synthetically.
...
We aim to find out how the diffusive transport of colloids across the pore is modified by the polymer brush. 
...
To this end, the colloid concentration is fixed to be zero and $\Delta c$ far away from the membrane (at $z\rightarrow\mp\infty$, respectively). 
...
Introducing the resistance $R$ to colloid flow as
\begin{equation}
J = \frac{\Delta c}{R}
\label{eq:resistance_definition}
\end{equation}
allows for a natural interpretation of Eq.~(\ref{eq:flux_finlength}) in terms of the total resistance of the pore:
\begin{equation}
    R = \frac{L}{D_0 \pi r_{\text{p}}^{2}} + \frac{1}{2 D_0 r_{\text{p}}} = R_{\text{int}}^{0} + R_{\text{ext}}^{0},
    \label{eq:resistance}
\end{equation}
where the superscript "zero" refers to the bare pore. The first term in Eq.~(\ref{eq:resistance}) represents the resistance of the interior of the empty pore, while the second term corresponds to the Rayleigh resistance for a pore of infinitesimal thickness (Eq.~(\ref{eq:flux_Ral})). The latter term accounts for the effects of convergent flow toward the pore entrance (in the region exterior to the pore) and its symmetric counterpart at the pore exit. However, inside the cylindrical pore, the flow lines are approximately axial. In the empty pore scenario, the inverse of the diffusion constant ($\rho_0=D_0^{-1}$) represents the resistivity of the medium both inside and outside the pore. 
Naturally,  the resistance of a pore in a thin membrane $L_{0} \ll r_{\text{p}}$ is determined by the resistance of the exterior region, $R_{0} \approx R_{\text{ext}}^{0}$.
In contrast, for long pores, where $L \gg r_{\text{p}}$, the resistance of the inner region becomes dominant, such that $R_{0} \approx R_{\text{int}}^{0}$.

Finite size of the solute particles affects the diffusive flux in two ways. To account for the excluded volume of diffusing colloids, assumed to be spheres with a diameter $d$, it is natural to use the effective pore radius $r_{\text{p}} - d/2\rightarrow r_{\text{p}} $ and the effective pore length $L +d \rightarrow  L$ \cite{Renkin1954, Beck1970, Bungay1973, Anderson1974, Brenner1977}. The second factor is of hydrodynamic nature and stems from the boundary conditions for the velocity field of the solvent leading to some additional drag \cite{Ladenburg1907, Faxen1922, Haberman1958}. However, the presence of the polymer brush screens the hydrodynamic effects of the pore walls and thus requires a different kind of drag analysis, as discussed below.
...
\begin{figure}
    ...
    Parameter values: $L=2r_{\text{p}}=56$, $\sigma=0.02$, $N=300$.
    }
...
%%%%%%%%%%
\subsection{Insertion free energy and diffusivity control the diffusive transport}
%%%%%%%%%%

Zooming in on the local scale, we can analyze how colloids are accumulated or depleted due to attractive or repulsive interactions, respectively, with the polymer, 
and how the polymer meshwork affects the particle local mobility.

%%%%%%%%%%
\subsubsection{Volume and surface contributions to the insertion free energy}
%%%%%%%%%%

The position-dependent insertion free energy $\Delta F(r,z)$ is the isothermal work required to move a colloid from exterior solution into the polymer brush.
...
and comprises two distinct contributions:
...
\noindent The coordinates $(r,z)$ refer to the center of the colloid, whilst the insertion free energy is obtained by integrating over the volume and surface of the colloid, respectively. All the free energy values are expressed in thermal energy units of $k_BT$.

The osmotic contribution, $\Delta F_{\text{osm}}(r,z)$, is proportional to the colloid volume 
and accounts for the work against excess osmotic pressure upon insertion of the particle into the brush. 
...
The interfacial contribution, $\Delta F_{\text{sur}}(r,z)$, is proportional to the particle area, 
...
\noindent Here $\gamma$ is the change in the free energy of a unit area upon replacement of a contact of the colloid with solvent by a contact with a polymer solution of concentration $\phi(r,z)$.
The coefficients $b_0$ and $b_1$ are parameters that account for depletion / accumulation of the polymer in the vicinity of the colloid surface, see Section 4 of the Supplementary information file.
...
where $\phi(r,z)$ is the polymer density distribution in a colloid-free brush calculated using the SF-SCF approach. 
...
Comparison of the approximate analytical approach for colloid with the SF-SCF simulations for colloids on the pore axis demonstrated good quantitative agreement (see Section 4 of the Supplementary Information, Figure~6), 
...
\begin{figure}
    ...
    Insertion free energies are displayed in cylindrical coordinates (as in Figure \ref{fig:phi_hm_grid}), and colour coded as indicated. 
    Pore and  brush parameters the same as in Figure \ref{fig:phi_hm_grid}, $\chi_{\text{PS}}=0.5$, $d=8$.
    }
...
Since the polymer concentration in the pore is strongly inhomogeneous 
the net insertion free energy $\Delta F(r,z)$  may exhibit quite large spatial variations. 
...
%%%%%%%%%%
\subsubsection{Particle mobility (diffusivity) is determined by the its size and the size of the polymer mesh}
%%%%%%%%%%
The local diffusion coefficient depends on the polymer volume fraction $\phi$ and the particle size: crowded polymer chains naturally decrease both rotational \cite{Fu2017} and translational \cite{Stewart1998} diffusion coefficients.
...
Brush-forming chains constitute an inhomogeneous semi-dilute solution with a concentration-dependent correlation length $\xi(\phi)$.
Particles of size $d > \xi$ experience additional friction as they are trapped by the polymer mesh network.
As a result, diffusion is slowed down compared to that in the pure solvent, leading to a position-dependent diffusion coefficient($D(r,z) < D_0$).
...
These models show the same qualitative trends in the diffusion coefficient as a function of particle size relative to the correlation length $d / \xi$.
...
Here we follow the model proposed by Cai \emph{et al.} \cite{Cai2011} to estimate the position-dependent diffusion coefficient as a function of particle size $d$ and polymer concentration $\phi$.
...
\noindent where the correlation length (mesh size) $\xi$ is controlled by the local polymer concentration $\phi(r,z)$. 
As follows from Eq. (\ref{eq:Rubinstein}), particles smaller than the mesh size diffuse virtually unimpeded ($D\{\phi(r,z)\}\approx D_0$ for $d\leq \xi$). 
In contrast, the diffusive motion of large colloids is significantly slowed by the polymer medium compared to the pure solvent ($D\cong D_0 (\xi/d)^2\ll D_0$ for $d\gg \xi$). 
...
We anticipate that even significant discrepancies between the employed model and the experimental data will not alter our qualitative conclusions. 
%UPDATE_MANUALLY%
This is demonstrated in Section 9 of the Supplementary Information, where we analyze the impact of the diffusion coefficient on pore resistance.
...
We develop an analytical method to estimate the resistance of a brush-filled cylindrical pore  to the diffusive flow of colloidal particles. 
...
Diffusive transport i the presence of external surface generated by insertion free energy $\Delta F$ is described by the Smoluchowski equation
\begin{equation}
...
\label{psi1}
\end{equation}
Under stationary conditions the flux density is divergence-free. In the case of position-independent diffusion coefficient and vanishing insertion free energy the potential function is a solution of the Laplace equation $\nabla^2 \psi(\bold r)=0$.
In our case the boundary conditions for the potential function are $\psi(z\rightarrow +\infty)=\Delta c$ and $\psi(z\rightarrow -\infty)=0$ since the insertion free energy $\Delta F$ vanishes far away from the pore.
 Hence problem of finding total resistance of two semi-infinite solution reservoirs separated by the membrane with the brush-filled pore is equivalent to finding the resistance of the medium with position- dependent conductivity 
\begin{equation}
\rho^{-1} (r,z)= D(r,z)\exp(-\Delta F(r,z))
\label{rho}
\end{equation}
To this end we chose a set of approximate equipotential surfaces that foliate the space available for the flow. Inside the pore they are taken as discs normal to the axis. Outside the pore we use the set of oblate spheroids taken from the Rayleigh solution \cite{Strutt1878}. 
The total conductivity of a layer between close equipotential surfaces can be found by integrating the local conductivities over the layer in analogy to the set of resistors connected in parallel. On the other hand since the  consecutive layers are connected in series, their total resistance can be found by appropriate integration: 
...
\begin{eqnarray}
    ...
    \label{eq:R_pore}
\end{eqnarray}
%In the exterior regions the resistance 
\begin{equation}
R_{\text{ext}} = 2\int_{L/2}^{\infty} 
(2\pi\int_{0}^{r_p}h_1h_2h_{3}^{-1})dr)^{-1}dz\\
\label{Rext}
\end{equation}
Here $h_1,h_2,h_3$ are the corresponding Lam\'e coefficients.
...
For an empty pore without a polymer brush, this method recovers the expression (Eq.~\ref{eq:resistance}) for the total resistance .
...
Conversely, a brush in good or $\Theta$-solvent conditions produces swollen fringes outside the pore (Figure~\ref{fig:phi_hm_grid}), which modify the resistance $R_{\text{ext}}$of the external regions.
...
As mentioned in the previous section, the resistance $\varrho(z)$ of a single layer (per init thickness as measured at the symmetry axis), is obtained by inverting the integral of the conductivity. 
For the brush-free region, $\varrho_{0}(z)$ is inversely proportional to the layer's surface area.

Figure~\ref{fig:R_map}A shows typical profiles of the resistance per unit length $\varrho(z)$ along axial coordinate $z$ 
%for a colloid particle with size $d=8$ 
at a few selected values polymer-colloid interaction parameters $\chi_{\text{PC}}$.
For comparison, we also show the case of an empty pore and  the case when the polymer brush affects only on the particle mobility, $\Delta F(r,z)\equiv 0$.
%as if the colloid particles have no interactions with the polymer brush but only experience increased viscosity of the medium.
...
Firstly, an attractive pore interior can enhance the diffusive fluxes through the polymer-filled pore beyond the limit of the empty pore.
...
However, this slowing down is counteracted by the attractive potential of the pore, which reduces local resistivity  according to the exponential factor in eq \ref{rho}.

Secondly, attractive brush fringes can further enhance the pore permeability by reducing local resistivity in the external region (see, e.g., the curves corresponding to $\chi_{\text{PC}} = -1.2$ and $-1.3$).
...
As the colloid attraction increases further $(\chi_{PC}\ll -1)$, the resistivity in the brush-filled regions effectively vanishes, 
%the resistance per unit length across the $z$ range spanning the pore interior and the caps becomes negligible, 
so that the convergent flow in the the polymer-free region remains the only source of resistance against colloid transport.
