...
Figure~\ref{fig:D_fe_map}a shows how the colloid diffusivity, $D(\phi)/D_{0}$, varies with the local polymer concentration for a particle of diameter $d = 8$; the corresponding spatial distribution $D(r,z)/D_{0}$ in a $\theta$-solvent ($\chi_{\text{PS}} = 0.5$) is presented in Figure~\ref{fig:D_fe_map}b.
...
Figure~\ref{fig:D_fe_map}a shows the net insertion free energy $\Delta F(\phi)$ of a colloid introduced into a homogeneous polymer mesh with polymer volume fraction $\phi$.
...
Above this concentration the osmotic contribution dominates and $\Delta F(\phi)$ becomes positive.
