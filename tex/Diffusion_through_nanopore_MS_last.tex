...
% This creates a solvated polymer meshwork that can fill the entire pore volume or remain confined to near-wall regions, depending on the polymer's molecular weight and conformational state.
% These polymer-modified mesoporous materials and membranes represent a new class of functional nanostructured systems with significant potential across a wide range of technologies.

% The interaction of (macro)molecules and nanocolloidal particles with porous media, as well as their transport through porous membranes, are key elements of many technological processes
% %ML: some of the references are about biological relevance either to be deleted or we should mention this relevance 
% (chromatography, heterogeneous catalysis, micro- and ultrafiltration, protein separation and purification, etc.) and have been studied intensively for more than sixty years \cite{Watson1959, Rout2003, Huang2023, Uredat2024}.
% %ML: also the sixty years passage is about nucleopores exclusively

Advances in macromolecular chemistry have enabled the functionalization of mesoporous materials (i.e., pores with a diameter between a few and many tens of nanometers) by anchoring polymer chains of various chemical nature to the pore walls.
This creates a solvated polymer meshwork that can fill the entire pore volume or remain confined to near-wall regions, depending on the polymer's molecular weight and conformational state.
These polymer-modified mesoporous materials and membranes represent a new class of functional nanostructured systems with significant potential across a wide range of technologies.

The interaction of (macro)molecules and nanocolloidal particles with porous media, as well as their transport through porous membranes, are key elements of many technological processes (chromatography, heterogeneous catalysis, micro- and ultrafiltration, protein separation and purification, etc.) and have been studied intensively for more than sixty years \cite{Watson1959, Rout2003, Huang2023, Uredat2024}. 
\todo{ML: some of the references are about biological relevance either to be deleted or we should mention this relevance, also the sixty years passage is about nucleopores exclusively}
...
\todo{RR: References in this paragaph cover mostly polymer brushes in contexts that are distinct from mesoporous materials. Their relevance is not quite clear, and I am lacking a strong argument for the potential of functionalised mesoporous membranes and the timeliness of our simulations in this regard?}
...
\todo{RR: One or two salient references are needed here. Azzaroni2012 might be one, but need to check, and perhaps also include some original research papers rather than not-so-recent reviews.}
...
Collectively, the FG domain meshwork provides remarkable gating function: biocolloids of just a tenth of the pore diameter or more in hydrodynamic diameter are effectively blocked, except for some dedicated transport factors (importins and exportins, alone and in complex with cargo) which bind to the FG domains and can undergo rapid permeation \cite{Hoogenboom2021}. 
...
Second, the NPC can gate diffusive colloid transport similarly well in both directions.
Whilst the native cell is capable of directed transport of cargo against a concentration gradient, this function is not intrinsic to the NPC and reversible through cell engineering without modifying the NPC structure \cite{Nachury1999}.
...
Fourth, studies with a spectrum of globular proteins as model colloids demonstrated that NPCs exhibit a wide and continuous spectrum of permeabilities as a function of colloid surface properties (at constant size \cite{Frey2018}) and size (for weakly or non-interacting proteins \cite{Ribbeck2001, Mohr2009, Popken2015, Timney2016}).
Instead, a fine balance of many individually weak physicochemical (e.g., electrostatic, hydrophobic, aromatic stacking, ...) interactions between polymers and biocolloids dictates the gating behaviour, rather than a few highly specific biochemical interactions \cite{Hoogenboom2021}.
...
Here, we develop a self-consistent field theoretical approach to address this question.
...
% The colloid concentrations are set to zero and $\Delta c$ far away from the membrane (at $z\rightarrow\pm\infty$, respectively).
% \todo{[FL: is this clear? .... it is zero at $z \to \infty$ and $c_0$ at $z \to -\infty$]}
The colloid concentrations are set to zero $c = c_0$ at $z \to -\infty$ and $c = 0$ at $z \to +\infty$ far away from the membrane.

%%%%%%%%%%
\subsection{Stationary flux}
%%%%%%%%%%
Diffusive transport in the presence of the external mean force generated by the position-dependent insertion free energy $\Delta F(\bm{r})/k_B T$ is described by the Smoluchowski equation,
\begin{equation}
    \frac{\partial c(\bm{r})}{\partial t}=-\nabla \cdot \left[D(\bm{r}) \nabla c({\bm{r}})+D(\bm{r})c({\bm{r}})\nabla(\Delta F(\bm{r})/k_{\text{B}}T\right],
    \label{eq:Smoluch}
\end{equation}
where $c(\bm{r})$ is the concentration of the colloid, $D(\bm{r})$ is its position-dependent diffusion coefficient. Here and below $k_{\text{B}}T$  is taken as the energy unit.
...
\end{equation}
The boundary conditions for the potential function in our case are $\psi(z\rightarrow -\infty)=\Delta c$ and $\psi(z\rightarrow +\infty)=0$ since the insertion free energy $\Delta F$ vanishes far away from the pore. The membrane itself is impenetrable, which means that the normal component of the flux density vanishes at the walls.
Once the insertion free energy field $\Delta F(\bm{r})$ and the position-dependent diffusion coefficient $D(\bm{r})$  are defined, a natural approach to find the total flux (resistance) is by direct numerical solution of Eqs.~(\ref{eq:D_tilde}-\ref{eq:Laplace_modif}) which we did as a check (see Supplementary Method 7). 
...
The insertion free energy $\Delta F=0$ everywhere, so that the potential function $\psi(\bm{r})$ coincides with $c(\bm{r})$, and the diffusion coefficient does not depend on the position.
...
\begin{equation}
    ...
    \label{eq:flux_Ral}
\end{equation}
where $r_{\text{p}}$ is the pore radius and $D_0 = k_{\text{B}}T / (3 \pi \eta_{\text{S}} d)$ is the diffusion coefficient of the colloid in plain solvent with viscosity $\eta_{\text{S}}$ ($k_{\text{B}}T$ is the thermal energy unit).
...
We neglect the latter as the presence of the polymer brush screens hydrodynamics and thus requires a different kind of drag analysis, as discussed below.%ML:Maybe put a reference here for that


%%%%%%%%%%%%%%%%%%
\subsubsection{Internal and External Resistances}
%%%%%%%%%%%%%%%%
...
The polymer density profile $\phi(z,r)$ in the pore was calculated by the two-gradient self-consistent field numerical method of Scheutjens and Fleer (SF-SCF; see Supplementary Methods 2).
...
For colloids that are significantly smaller than the size of the pore, the insertion free energy is determined entirely by the local polymer concentration (i.e., wall effects can be neglected), and comprises two contributions:
\begin{eqnarray}
...
The coefficients $b_0 = 0.7$ and $b_1 = -0.3$ are phenomenological parameters that account for depletion or accumulation of the polymer in the vicinity of the colloid surface, and can be treated as constant to a good approximation (Supplementary Methods 3-4).
...
With this scheme, we can evaluate the insertion free energy at any position of the colloid in the brush, including off the pore axis (Supplementary Method 5).
Comparison with direct SF-SCF calculations of the insertion free energy for colloids on the pore axis demonstrated good quantitative agreement (Supplementary Method 4), justifying the use of the more versatile analytical scheme.
...
In section \ref{nonsticky} \todo{ML: we cannot refer to subusubsection, as it will not have number because of the Author Guidelines}, we estimate $\beta = 5.5$ by comparing our theoretical stationary flux predictions to experimental data on the flux of non-sticky colloids of different sizes through nuclear pore complexes.
...
Figure~\ref{fig:D_fe_map}a (black line) shows how the colloid diffusivity, $D(\phi)/D_{0}$, varies with the local polymer concentration for a fixed colloid size ($d = 8$); the corresponding spatial distribution $D[\phi(z)]/D_{0}$ in a $\theta$-solvent ($\chi_{\text{PS}} = 0.5$) is presented in Figure~\ref{fig:D_fe_map}b.
\todo{FL: say a few words that D/Do decreases monotonically (-ln increases of course)}
...
However, in section \ref{sticky} we return to this question in the context of proteins traversing the nuclear pore, where the colloid surface and the polymers may be heterogeneous with affinity localized in a few sticky patches.
...
\begin{figure}
    ...
    % \todo{RR: I think these data are better shown as a Supporting Figure, that comprises just $R$ computed through the two different methods.}
    }
...
Colloid concentration profiles under stationary flux conditions can be found by numerically solving Eq.~(\ref{eq:Smoluch}) with $\frac{\partial c(r,z)}{\partial t} = 0$ (Supplementary Method 7).
...
\todo{FL: I assume that you expect that the colloid-colloid interaction inside the brush is mainly repulsive... However, I am not fully sure about this. There might be colloid-colloid attraction and then your conclusion might be premature.....}
...
To test our theory, we analysed literature data pertinent to colloid transport across NPCs. The average distance between NPCs in the nuclear envelope exceeds the pore diameter by almost an order of magnitude \cite{Winey1997, Varberg2022, Maeshima2006}. Transport across neighbouring NPCs thus is not mutually interfering \cite{Fabrikant1985}, as can be appreciated from the iso-concentration lines in Figure~\ref{fig:colloid_concentration}. Experimentally measured transport rates $k$, normalized against the number of pores, therefore can be directly compared with our theoretical predictions.

It is well-known that colloids with affinity for the disordered nucleoporin FG domains that fill the NPC (such as importins and exportins) are enriched in or near NPCs \cite{Beck2007, Lowe2015, Kim2018, Gruenwald2010, Tu2011}, and in microscopic droplets, macroscopic hydrogels and thin films assembled from pure FG domains \cite{Schmidt2015, Zahn2016, Vovk2016, Frey2018}.
Moreover, high concentrations of transport factors promote effective transport through the pore \cite{Lowe2015}.
...
\todo{RR: Gruenwald2010 does not seem to show what is claimed? Tu2011 also is somewhat indirect regarding accumulation of importins and exportins?}
...
The effective statistical segment length of disordered polypeptide chains was taken to be $a$ = 0.76 nm. \todo{RR: Ralf and/or Mikhail to add reference.}
The effective solvent strength in the NPC was estimated to be close to $\theta$-solvent ($\chi_{\text{PS}} = 0.6$), consistent with varying yet generally moderate levels of 'cohesiveness' observed for FG domains. \todo{RR: Ralf and/or Mikhail to add references.}
...
The satisfactory fit across all datasets suggests that the crowding of the NPC with transport factors has at best a weak effect on the transport of non-sticky proteins across the NPC \todo{RR: There may be experimental work that has looked at this}.  
...
\begin{figure}
    ...
    The pore and brush parameters are as given in (a); $\chi_\text{PC}$ values (indicated at the top of the graph) were matched to the partition coefficient $P$.
    % \todo{RR: May link to a Supporting Figure or Table that explains how this was done?}
    }
...
To reproduce theoretically the correlation between the experimentally measured NPC transport rates $k$ and the partition coefficient $P$ in pure FG domain phases, we determined the effective solvent strength for each FG domain $\chi_\text{PC}$ from the experimentally measured volume fractions of the FG domain in its polymer-rich droplets formed upon spontaneous phase separation \cite{Frey2018} (see Supplementary Methods 8).
\todo{RR: Please check if correct, or justify why vanishing osmotic pressure is a sensible assumption}
\todo{ML: SI The estimated critical concentration for Nup116 and Nup98A FG-domains is $1 \, \mu\text{g}/\text{ml}$, corresponding to \mbox{$\phi_{\text{gel}}^{\text{out}} \approx 10^{-6}$} \cite{Schmidt2015}. This extremely low value implies the osmotic pressure is effectively zero in both phases. SI}
...
\todo{RR: Please check if correct, or justify why vanishing osmotic pressure is a sensible assumption}, and provides the link between $\chi_\text{PC}$ and the partition coefficient. 
...
\todo{RR: Ralf to check and add references.}
