...
\todo{Figure~\ref{fig:D_fe_map}a shows how the colloid diffusivity, $D(\phi/D_{0}$, varies with the local polymer concentration for a fixed colloid size; the corresponding spatial distribution $D(\{\phi(r,z)\})/D_{0}$ in a $\theta$-solvent ($\chi_{\text{PS}} = 0.5$) is presented in Figure~\ref{fig:D_fe_map}b.}
...
\begin{figure}
    ...
    \caption{%
        ...
        so the horizontal zero line corresponds to unimpeded transport in pure solvent; positive values are raised resistance, whereas negative values are lowered resistance.
        ...
        \\  
        In all panels the pore and brush parameters are those of Figure~\ref{fig:colloid_transport}; the solvent strength parameter is $\chi_{\text{PS}}=0.5$ and the particle diameter is $d=8$.%
    }
    \label{fig:D_fe_map}
\end{figure}
...
\todo{%
Because the polymer volume fraction $\phi(r,z)$ varies throughout the pore, the local resistance $\rho(r,z)$ is likewise position-dependent.
Figure~\ref{fig:D_fe_map}a separates the two underlying contributions:
...
\begin{figure}
    ...
    The intrinsic orthogonal curvilinear coordinates are defined by equipotential surfaces, \mbox{$\psi = \text{const}$}, and the flux lines of flux density field $\bm{j}$.
...
\begin{figure}
    \centering
    \includegraphics[width=3.5in]{fig/streamlines.png}
    \caption{
...
\begin{figure}
    ...
    \todo{Thin black line is no excluded volume effect, so only Stokesian scaling}
    }
    \label{fig:R_vs_d}
\end{figure}
...
To match the theoretical colloid volumes to protein molecular masses, we considered the effective density of the protein colloids to be bounded by the densities of aqueous solvent ($\rho_{\text{gel}} \geq \text{1 g/cm}^3$) and pure polypeptide ($\rho_{\text{gel}} \lesssim \text{1.4 g/cm}^3$).
