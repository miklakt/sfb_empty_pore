...
\label{eq:varrho2}
\end{equation}
where $r'(r,z) , z'(r,z)$ are the functions parameterizing the equipotential surfaces, and $h_r$, $h_{\theta}$ and $h_z$ are the corresponding Lam\'e coefficients (see Supporting Information, Section 7). In the case of a homogeneous brush considered here, the function $\varrho_{\text{ext}}^{-1}(z)$ is even.%, which defines it for $z<-L/2$.
...
Figure~\ref{fig:R_vs_d} illustrates how the total resistance varies with colloid size at a different combination of interaction parameters, a fixed solvent strength, $\chi_{\text{PS}}$ in a panel (a), or at a fixed polymer-colloid interaction strength, $\chi_{\text{PC}}$ in panel (b).
...
Aside from these effects, the parameters influencing the total resistance are the same as those in Eq.~(\ref{eq:rho}), as the total resistance is a result of integration of position-dependent conductivity.
...
Upon an increase in $\chi_{\text{PS}}$ as a polymer deswell both minimal and maximum polymer concentration $\phi$ increase, so the equivalent concentration follow the same trends with respect to solvent quality $\chi_{\text{PS}}$ as average concentration in the pore $\tilde{\phi} \approx \left<\phi\right>$.
%Because of the exponential term in the Eq.~(\ref{eq:rho}) the averaging correspond mostly to the one of convex function \emph{i.e.} is closer to the maximum value of $\phi$.

Terms (i), (ii), and (iii) in Eq.~(\ref{eq:R_contrib}) correspond to the effects of slowed diffusion, osmotic penalty, and surface contribution to resistance, respectively.
...
\begin{equation}
    \begin{aligned}
        &\frac{\partial \ln R}{\partial \ln d} = \frac{\partial R}{\partial d}\frac{d}{R}  \approx 
        \\
        &
        =\underset{\text{(i)}}{ 1 +\frac{2 \alpha \phi^{2} d^{2}}{\alpha \phi^{2} d^{2} + 1}}
        \underset{\text{(ii)}}{+ \frac{\pi \Pi\{\tilde{\phi}\} d^{3}}{2}}
        \underset{\text{(iii)}}{\vphantom{\frac{a}{a}} + 2 \pi \gamma\{\tilde{\phi}\} d^{2}}
...
For extremely small particles $d \to 0$, all the term that depend on particle size $d$ in Eq.~(\ref{eq:R_contrib_deriv}) cancels out and the relative rate of change is $\frac{\partial \ln R}{\partial \ln d} \gtrapprox  1$, signifying Stokesian behavior of small solutes, irregardless of interaction parameters $\chi_{\text{PS}}, \chi_{\text{PC}}$.
...
% Positive insertion free energy landscape will make the dependency steep superlinear with $\frac{\partial \ln R}{\partial \ln d} > 3$, while negative insertion free energy landscape will diverge the curves to sublinear behaviour $\frac{\partial \ln R}{\partial \ln d} < 3$ up to a negative slope.
...
With $\chi_{\text{PC}} > -1.0$ curls up 

\paragraph{Effect of polymer-colloid interacting strength}
An increase in polymer-colloid interaction strength (decrease in $\chi_{PC}$) is followed by an increase in $\gamma$ as in Eq.~(\ref{eq:chi_ads}).
The resistance of the polymer-filled pore systematically drops for every particle size $d$ as each curve in Figure~{\ref{fig:R_vs_d}}a shifts down with decreasing $\chi_{\text{PC}}$.
Low polymer-colloid interaction strength $\chi_{PC} > \chi_{\text{ads}}$ defines positive insertion free energy landscape (see Figure~\ref{fig:DeltaF_map} for $\chi_{PC} \ge -0.75$) making the (ii) and (iii) terms in Eq.~(\ref{eq:R_contrib_deriv})) positive, as seen in Figure~\ref{fig:R_vs_d}a for $\chi_{PC} > -1$ as superlinear dependency $\frac{\partial \ln R}{\partial \ln d} >3$.
Thus a polymer filled pore has higher selectivity to particle size for particles with low polymer-colloid interaction strength.

On the other hand, a decrease in $\chi_{\text{PC}}$ below $\chi_{\text{ads}}$ lead to a complex non-monotonic dependency.
For any negative $\gamma$ the (ii) and the (iii) terms in Eq~(\ref{eq:R_contrib}) defines a cubic parabola as a function of particle size $d$ with a minimum ($d=0$) followed by a maximum ($d = -\frac{4\gamma}{\Pi}$), or an inflection point ($d=0$) in the case $\gamma = 0$.
Given an always positive, slower growing logarithmic term (i) in Eq~(\ref{eq:R_contrib_deriv}) inflection point at some particle size $d$ appear at some small negative $\gamma < 0$ rather then zero.
With further decrease in $\gamma < 0$ a maximum ($d>0$) followed by a minimum ($d < -\frac{4\gamma}{\Pi}$) appear, the width of the section of the curve with negative slope $\frac{\partial \ln R}{\partial \ln d}$ grows with more negative $\gamma$.
This section of the curve signifies an unexpected behavior for the polymer-filled pore when similar homogeneous colloid particles of larger size have less resistance then smaller ones.
The mentioned extrema is seen on the curve in Figure~\ref{fig:R_vs_d}ab for a set of interaction parameters $\chi_{\text{PC}} = -1.25$ and $\chi_{\text{PS}} = -1.25$ denoted with up arrows as (a) and (b) for maximum and minimum, respectively.
The selectively with respect to particle size near the extrema is low.
% Upon an increase in $\chi_{\text{PS}}$ (decrease in solvent strength) or a decrease in $\chi_{\text{PS}}$ (increase in polymer-colloid interaction strength) the behavior is changed from strictly monotonic superlinear to non-monotonic with sub- and superlinear sections.

The selectivity with respect to polymer-colloid interaction increases with particle size and decrease in solvent strength (higher $\chi_{\text{PS}}$, thus higher $\tilde{\phi}$)
%$\frac{\partial \ln R}{\partial \chi_{\text{PC}}} = \pi d^{2} \frac{\partial \gamma}{\partial \chi_{\text{PC}}} \approx  \frac{\pi d^{2}}{6}\tilde{\phi}$
$\frac{\partial \ln R}{\partial \chi_{\text{PC}}} \approx  \frac{\pi d^{2}}{6}\tilde{\phi}$.


\paragraph{Effect of solvent strength}
Similarly, an increase in solvent strength (increase in $\chi_{PS}$) is followed by an increase in $\gamma$ as in Eq.~(\ref{eq:chi_ads}) and decrease in the osmotic pressure $\Pi$ as in Eq.~(\ref{eq:osmotic}).
For larger particles $d>4$ an decrease in $\chi_{PS}$ is qualitatively the same as an increase in $\chi_{\text{PC}}$, which drives both osmotic and surface term to the insertion free energy landscape more positive.

On the contrary, as with increase in $\chi_{PS}$ the polymer deswells, the equivalent concentration $\tilde{\phi}$ in the pore lumen rises.
For smaller particles the effect of hindered diffusion (term (i) in \ref{eq:R_contrib_deriv}) dominates, so that $\frac{\partial \ln R}{\partial \chi_{\text{PS}}} \approx  < 0$ is small and negative, the slope of $R(d)$ curves does not depend on solvent quality $\frac{1}{\partial \chi_{\text{PS}}}\frac{\partial \ln R}{\partial d} \approx 0$.
As seen in Figure~\ref{fig:R_vs_d} as the curves closely vertically stack on each other with the same slope in the order of decreasing $\chi_{\text{PS}}$ in the range of smaller particles $d<4$.
For larger particles term (ii) and (iii) in \ref{eq:R_contrib_deriv} dominate with the curves diverge.

\paragraph{Effect of the reservoir}
For very low insertion free energies $\Delta F \ll 0$ when $\gamma < 0$ as the interior region is essentially short-circuited $R_{\text{int}} \to 0$ the total resistance of the pore is defined by always finite resistance of the exterior region $R \approx R_{\text{ext}}$ as in Eq.~(\ref{eq:R_tot_tot}) and Figure~\ref{fig:resistivity_profile}.
At the same time in the case of very attractive particles, the fringes of formed by the swelled polymer may have very low resistance, decreasing the resistance of the exterior region.
Following Eq.~(\ref{eq:r_dome}), $\frac{\partial \ln R}{\partial \ln d} \approx  1$.

Upon an increase in particles size when $\gamma$ is sufficiently negative, as we discussed before, we expect a minimum, as one denoted with uparrow (b) in Figure~\ref{fig:R_vs_d}a.
However, the total resistance is caped, at some particle, the expected parabolic-like section is almost piesewise-like substituted with linear slope.
The onset of the the linear slope is a minimum is marked with uparrow (c) in Figure~\ref{fig:R_vs_d}a.
Upon further increase in particle size as the osmotic penalty grows and so does membrane-particle interference constrains, the linear slope is substituted back by the rising branch of parabola-like curve.
...
% % It is known that even relatively large colloid particles that are considered inert may leak through a nucleopore (\totdo Ref Popen et al ), which can be attributed...
