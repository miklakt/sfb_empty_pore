...
\begin{equation}
    ...
    \label{eq:flux_psi}
\end{equation}
Under stationary conditions, the original Smoluchowski equation (Eq.~\eqref{eq:Smoluch}) is reduced to
\begin{equation}
...
\label{eq:varrho2}
\end{equation}
where $r'(r,z) , z'(r,z)$ are the functions parameterizing the equipotential surfaces, and $h_r$, $h_{\theta}$ and $h_z$ are the corresponding Lam\'e coefficients (Supplementary Note 2). In the case of a homogeneous brush considered here, the function $\varrho_{\text{ext}}^{-1}(z)$ is even.
...
The coefficients $b_0 = 0.7$ and $b_1 = -0.3$ are phenomenological parameters that account for depletion or accumulation of the polymer in the vicinity of the colloid surface, and can be treated as constant to a good approximation (Supplementary Note 4).
...
\todo{RR: Please check if Supplementary Note 6 is appropriately referenced here, as I am not sure.}
...
To test our theory, we analyzed literature data pertinent to colloid transport across NPCs.
...
\begin{figure}
    ...
    The pore and brush parameters are as given in (a); $\chi_\text{PC}$ values (indicated at the top of the graph) were matched to the partition coefficient $P$, $d$ = 6 (Supplementary Note 7).
