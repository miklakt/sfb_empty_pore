...
The polymer density profile $\phi(r,z)$ in the pore was calculated by two-gradScheutjens
...
In the present paper we present a theory to explain physical principals that governs the diffusive transport of colloids through mesopores with a polymer brush grafted to the inner surface of the pore.
Typically, particles are expelled by the polymer brush because the compression of polymer chains increases the osmotic penalty, leading to a repulsive force that drives the colloid out.
Moreover, polymer brush creates a dynamic meshwork that obstruct colloid diffusion through the brush. 
This two effects hinder colloid transport.
We showed that non-specific adhesive interactions of colloid particles with polymer chains can overcome osmotic expulsion and facilitate colloid transport through the pore.
Notably, the presence of polymer brush may lead to lower resistance to diffusive transport $R$ than a bare pore with no brush $R_{\text{empty}}$ and even bring resistance of the pore below the resistance of the pore in an infinitely thin membrane $R_{\text{pore, empty}}$ (orifice).
We showed that polymer brush mesopores are permeation selective to the particle affinity to polymer.
They also exhibit higher selectivity to particle size compared to a bare pore.

We explored how parameters such as solvent quality, particle size and its adhesion to polymer determine flux through the pore (or resistance) and pore selectivity.
We provide an analysis of the pore resistance $R$ as the sum of the resistances of the pore channel $R_{\text{pore}}$ and exterior regions $R_{\text{conv}}$ wich associated with resistance caused by convergent flow.
We showed that the pore filling modulates the resistance of the pore channel $R_{\text{pore}}$ and the regions outside the pore.
To better access the results a comparison with resistance of a bare pore $R_{\text{empty}}$ as a reference were made.
The results may provide a better understanding on the facilitated permeation of biomacromolecules from cytoplasm to nucleoplasm through nucleopores that perforate nuclear envelope.

In the present paper we studied a scenario where one cylindrical pore perforate finite thickness membrane that separates two semi-infinite reservoirs.
The pore is with grafted freely-joined polymer chains grafted to the inner surface.
The colloid fluxes were analyzed for the case when colloid concentration is fixed far away from pore to some finite value on one side and zeroed on the other.
...
The resultant effect of the pore interference is an increase in the pore permeability since the 
%Rayleigh resistance%
resistance of the exterior region is partially shared by the neighboring pores. 
...
It is intuitively clear that once the resistance due to a finite pore length and due to the brush is non-negligible, the mutual interference effect becomes even smaller.
Hence, we are not concerned with this aspect of the problem.

The polymer concentration distribution $\phi$ (concentration of the segments of the brush-forming polymer chains) were studied with Scheutjens-Fleer self-consistent field theory for different solvent qualities.
The solvent quality were quantified with Flory-Huggins solubility parameter.
We the upon swelling polymer brush occupies space outside the pore, which modulates the resistance of the exterior regions.

Particle adhesion to the polymer is control by Flory-Huggins interaction parameter, together with particle size and local polymer concentration it control insertion free energy $\Delta F$.
A free energy penalty associated with embedding a particle in the brush.
In the paper we analyze insertion free energy landscape, and showed how, triggered by the solvent strength,particle affinity or particle size it can switch from energy well dominated by favorable adhesive interaction to a energy barrier created by osmotic repulsion.

The research employed scaling theory to assess the localized slowdown of particle diffusion
due to polymer entanglement in the brush. 
For particles smaller than the polymer mesh’s correlation
length, diffusion was largely unhindered, while larger particles experienced hopping
diffusion—temporarily trapped within the mesh and awaiting relaxation of polymer chains before
moving to the next position.
This dynamic resulted in a slowed diffusion rate for larger particles $D/D_0 < 1$.

Local insertion free energy and local diffusion coefficient is associated with local conductivity $D \exp(\frac{-\Delta F}{k_B T})$, b, we developed an approximate integration scheme to find resistance of the polymer brush filled pore from the local conductivities analytically.

To validate our approximate analytical solution, we have also performed full numerical solution of the Smoluchowski diffusion equation, which is illustrated by 
%%%%%%UPDATE_MANUALLY%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Figure 14 od Supplementary Information
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
and corresponding point are presented, together with analytical results in Figure \ref{fig:R_vs_d}.
A very good quantitative agreement proves accuracy of our analytical theory
within the limit of the particle size sufficiently smaller than the pore radius. 

Here are some key findings:

Polymer brush slows down inert or slightly adsorbant colloid particles.
As it is seen from Figures \ref{fig:R_vs_chi_PC}, \ref{fig:R_vs_d} particles with interaction parameter $\chi_{\text{PC}} \gtrsim -1$ experience higher resistance compared to a bare pore for particle every size.
For $\chi_{\text{PC}} \gtrsim -0.75$ interfacial term is positive $\gamma \gtrsim 0$ with insertion free energy dominated by osmotic repulsion term $\Delta F > 0$.
As seen from the Figure \ref{fig:R_vs_d} larger particles are effectively blocked, with resistance on the particle size systematically beyond the case of bare pore and grows exponentially with particle volume $R \approx e^{\Delta F} \approx e^{\Pi V}$.

For the facilitated permeation (resistance lower than the resistance of bare pore) is assured for sufficiently attractive particle, when there is a path with negative insertion free energy in free energy landscape (as seen in Figure \ref{fig:DeltaF_map} for $\chi_{\text{PC}} \le -1.00$).
\begin{equation*}
    \forall z \exists r, \Delta F(r,z) < 0
\end{equation*}
But the existence of negative insertion free energy itself does not necessarily result in facilitated permeation.
There are two effects that it has to overcame wich happen at sufficiently high $\chi_{\text{PC}} < \chi^{\text{crit}}_{\text{PC}}$ adhesion: (i) slowdown because of the steric hindrance of polymer meshwork, and (ii) path narrowing either because of volume exclusion or the shape of energy landscape. 
This was analyzed and exemplified in Figure \ref{fig:R_map}, where while $\chi_{\text{PC}} = -1.00$ at $\theta$-solvent ($\chi_{\text{PS}} = 0.5$) assures negative insertion free energy path (as seen in Figure \ref{fig:DeltaF_map}) while fails to provide facilitated transport (i.e. $R > R_{\text{empty}}$).
Notably, a slight change in $\chi_{\text{PC}}$ towards more attractive particle is enough to ensure facilitated permeation $R < R_{\text{empty}}$.

Larger particles requires higher affinity to the polymer $\chi_{\text{PC}} < \chi^{\text{crit}}_{\text{PC}}$, as the positive osmotic term grows with particle volume $\Delta F_{\text{osm}} \sim d^3$, while negative interfacial term grows with the particle surface $\Delta F_{\text{osm}} \sim d^2$ as we demonstrated in the Figure \ref{fig:R_vs_chi_PC} the $\chi^{\text{crit}}_{\text{PC}}$ curve intersections with resistance on particle affinity curves shifts lefts with increasing particle size and in Figure \ref{fig:R_vs_d} where intersection of resistance on particle size curves intersects with the reference curve of a bare pore are shifted to the larger particles for larger $\chi_{\text{PC}}$.

Polymer brush pore permeability is limited by the exterior region, i.e. there is a lower boundary to the total resistance of the pore.
While this is subject to the choice of boundary condition, it is observed for any system with a pore where diffusing particle has to travel through sufficiently long path through pure solvent by the means of diffusion.
Which is also the case for nucleopore [\dots].
Notably, upon change in solvent strength as polymer brush swells it occupies more and more space in the exterior region (as seen in Figure \ref{fig:phi_hm_grid}), the portion of path that particle has to travel passively through the pure solvent decreases, which modulates resistance of the exterior region $R_{\text{conv}}$.
For the case of a very large negative insertion free energy $\Delta F \ll 0$ up to infinitely negative $\Delta F \to  -\infty$ 
total resistance is limited by the resistance of the exterior region $R_{\text{pore}} \to 0$, $R_{\text{conv}} \to \frac{1}{D_0 2 \pi r_{\text{ext}}}$ 
where $\frac{1}{D_0 2 \pi r_{\text{ext}}}$ is twice the resistance to diffusion flow for particle diffusing from a semi-infinite solution to an ideally absorbing sphere of radius $r_{\text{ext}}$ that circumference the exterior to the pore space where polymer brush is still found.

As local conductivity is exponential to local insertion free energy, total resistance is also connected to insertion free energy landscape exponentially. the consequence of the this is large sensitivity of pore resistance to parameters that influence insertion free energy such as particle size and affinity.
This defines very high selectivity of polymer brush to these parameters as demonstrated in Figures \ref{fig:R_vs_chi_PC}, \ref{fig:R_vs_chi_PC}.
The dependency of insertion free energy on particle size is cubic, thus for a particles near some critical size $d_{\text{crit}}$ a slight change in particle size translated to drastic change in permeability (resistance).
The permeation selectivity to particle size increases with $d_{\text{crit}}$ wich increases with particle adsorption strength, decrease in $\chi_{\text{ads}} \sim \chi_{\text{PC}}$ up to a behavior recognized as "gating".
This demonstrated in Figure \ref{fig:R_vs_d}, with step-like change in resistance for particles with high affinity $\chi_{\text{PC}}$ and large $d_{\text{crit}}$ on a minor change in particle size, with the resistance switching from below bare pore to greatly larger.
Upon decrease in solvent strength (increase in $\chi_{\text{PS}}$) the size that corresponds this switching move to the larger particles as it increases adsorption strength (decreases $\chi_{\text{ads}}$).
For the same reasons, larger particles with affinity near $\chi^{\text{crit}}_{\text{PC}}$ experience gating behavior of the pore with respect to particle affinity, as the slight change in $\chi_{\text{PC}}$ brings insertion free energy from largely negative to largely positive.
Again, decrease in solvent strength (increase in $\chi_{\text{PS}}$) shifts $\chi^{\text{crit}}_{\text{PC}}$ to a larger values, as in poorer solvent increases adsorption strength (decreases $\chi_{\text{ads}}$).

Finally, facilitated permeability implies increased concentration of colloid particles in the polymer brush.
This is follows straightforward from the fact that facilitated permeation requires negative insertion free energy, that cause colloid partitioning inside the brush.
The resulting colloid concentration in steady-state $c/c_{text{b}}$ is not the same as for equilibrium state $c/c_{text{b}}$, but come closer as insertion free energy became largely negative.
We present a selected case of colloid crowding in a steady-state in Figure \ref{fig:colloid_concentration}.
Interestingly, high partitioning of diffusing specie in the brush does not necessarily mean high permeation rate. 
For example, in a $\theta$-solvent, interaction parameter $\chi_{\text{PC}} = -1.00$ ensures negative insertion free energy (as seen in Figure \ref{fig:DeltaF_map}) with increased concentration of diffusing species near the entrance $c/c_{text{b}} >1$ to the pore while it does not provide facilitated permeation $R<R_{\text{empty}}$ (as seen in Figure \ref{fig:R_map}).
In the other words, high partitioning requires negative insertion free energy $\Delta F(r,z) < 0$, while as we mentioned earlier, facilitated permeation requires a path with negative insertion free energy $\forall z \exists r, \Delta F(r,z) < 0$.
