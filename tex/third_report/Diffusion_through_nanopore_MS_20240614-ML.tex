...
As we already discussed, protruding attractive polymer brush can bring the resistance even below the resistance of an infinitely thin empty pore $R<R_{\text{empty, conv}}$, in this case the total resistance is defined by the resistance of the semi-infinite reservoir which is not modulated by the pore.
Hence, you can see the black lines plateauing at sufficiently negative $\chi_{\text{PC}}$.

For larger particles the plateaus in Figure~\ref{fig:R_vs_chi_PC} are followed by the lines with a steep slope, so that even tiny change in polymer-colloid interaction $\chi_{\text{PC}}$ result in a large change in the permeability. 
For larger particles the plateaus in Figure~\ref{fig:R_vs_chi_PC} are followed by the lines with a steep slope, so that even tiny change in polymer-colloid interaction $\chi_{\text{PC}}$ result in a large change in the permeability. 
The change from plateau to steep slope implies gating behavior of a pore with polymer brush with respect to polymer-colloid interaction $\chi_{\text{PC}}$.

To analyze the pore resistance in Figure~\ref{fig:R_vs_chi_PC} we trace $\chi_{\text{PC}}^{\text{crit}}(d)$ (blue curve) that bisects the lines of total resistance in into region were $R<R_{\text{empty}}$ when $\chi_{\text{PC}} < \chi_{\text{PC}}^{\text{crit}}$, and a region with impeded permeation $R>R_{\text{empty}}$.
While the region to the right of blue curve is characterized with high selectivity to polymer-colloid interaction $\chi_{\text{PC}}$, it is also a region with high pore resistance and low colloid flux.
In this sense, the traced $\chi_{\text{PC}}^{\text{crit}}(d)$ defines pore's operative range.

Notably, the gating behavior of a pore is coupled with enhanced colloid transport.
