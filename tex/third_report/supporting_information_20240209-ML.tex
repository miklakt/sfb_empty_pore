...
%$\textrm{Pe}_{\textrm{n}}$ & the Péclet number for a flux between neighboring control volumes in the given direction \\
...
These effects have been studied, and several models have been proposed~[refs\dots].
...
\begin{figure}[H]
    \centering
    \includegraphics[width = 0.75\linewidth]{fig/roadmap.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[scale = 0.8]{fig/sf-scf_scheme.png}
    \caption{
...
All the Markov processes that start with segment $s_i$ and end with segment $s_k$ at coordinates $\{r, z\}$ are found as the sum over all possible starting coordinates:
\begin{equation}
    \label{eq:sum_to_phi}
...
Each iteration returns a new improved approximation to the segment potential $\mathbf{u}$ and updates the volume concentrations.
...
Let us start with the continuous form; the analytical insertion free energy for a cylindrical particle coaxial with the $z$-axis can be written as follows:
...
We remind that the SF-SCF method utilizes space discretization; inherently, the polymer brush concentration and all derived properties are defined on a discrete two-dimensional grid and can be represented as matrices, for example, $\phi(z, r)$ and $\boldsymbol{\phi}$ indexed with $i$ and $k$.
...
Let us switch to integration on the cylindrical lattice.
...
\begin{eqnarray}
    ...
    \label{eq:Delta_F_V} \Delta F_{\textrm{osm}} &=& \int_{V} \Pi(z, r) \, dV \\
    \label{eq:Delta_F_S} \Delta F_{\textrm{sur}} &=& \oint_{S} \gamma(z, r) \, dS
\end{eqnarray}
...
For each of these series, we performed a ground state free energy correction, meaning that the total free energy for a particle far away from the pore is subtracted to ensure the reference value $\Delta F(z_c \to \pm \infty) = 0$.
...
These results diverge from the SF-SCF predictions for inert particles ($\chi_{\textrm{PC}} = 0$) and strongly adsorbing particles ($\chi_{\textrm{PC}} = -1.5$).
...
Larger particles cause polymer brush compression, which translates to an additional free energy penalty not accounted for by the analytical approach; this effect is seen in Figure~3 of the main text.
...
\begin{figure}[H]
    ...
    \caption{Comparison of fitted and non-fitted results with SF-SCF for $d = 4$}
    \label{fig:fit_SI}
\end{figure}
...
The identities used in the later expressions are:
...
\end{gather}
where $\bm{J}$ is the Jacobian matrix, and $\hat{n}$ is a normal to the surface.
...
\begin{eqnarray}
    ...
    \end{split}
\end{eqnarray}
where $dA = \frac{\left|\frac{\partial \bm{T}}{\partial r} \times \frac{\partial \bm{T}}{\partial z} \right|}{\left|\hat{n}(r, z, \theta) \cdot \frac{\partial\bm{T}}{\partial \theta}\right|} dr dz$ is differential element of area.
...
To find the elements of the projection matrices, we discretize the projections by integrating over a domain $r, z \in [i, i + \delta r] \times [k, k + \delta z]$:
...
\begin{figure}[H]
    ...
    \caption{
        ...
        In the figure, we exemplify the volume projection matrix $\bm{V}\{r_c\}$.
        The colored tiles encode the matrix elements' values, where yellow means larger values and dark blue means lower values.
        The geometrical meaning of the matrix element is the particle volume (red body) or surface (green small patch) found in the domain $[i, i + \delta r] \times [k, k + \delta z]$ (opaque blue toroid).
...
The morphological dilation in this case is the locus of the points covered by a sphere when the center of the sphere moves inside the membrane body.
...
As the flux intensity is a conservative vector field, we introduce a scalar field $\psi$, such that $\mathbf{j} = -D \nabla \psi$.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{fig/coordinate_system.png}
    \caption{
        (Left)
...
For a thin empty pore, this curvilinear coordinate system is a variant of the oblate spheroidal coordinate system, where surfaces of constant $x_{\theta}$ are half-planes, surfaces of constant $x_{r}$ are confocal hyperboloids of revolution, and surfaces of constant $x_{z}$ are confocal oblate spheroids.
...
We assume that in the selected coordinate system, the flux is always normal to constant $x_z$ surfaces, which is exact for point-like particles passing through an infinitely thin empty pore.

In our previous paper~[reference], we noted that the product $\rho = D e^{-\Delta F / k_B T}$ has the meaning of local conductivity.
Then, integration over the constant $x_z$ surfaces gives the inverse resistance per unit length (as appropriate for resistors connected in parallel), and the integration over each $x_z$ coordinate simply adds contributions from all the slices connected in series.
...
The total resistance is the sum of the resistances of all the $x_z$ layers.
...
\begin{eqnarray}
    ...
    R_{(z, \pm\infty)} = \pm \int_{z}^{\pm\infty} R_z \, dx_z = \dfrac{\pi - 2 \arctan\left( \dfrac{|z| - L/2}{r_{\textrm{pore}}} \right)}{4 \pi r_{\textrm{pore}}}
\end{eqnarray}
...
To update the average colloid concentration $c$ in a control volume, we use the forward Euler method:
\begin{equation}
...
Identical to the previous section, we use a discrete cylindrical lattice; we remind that as the system has axial symmetry, the lattice is two-dimensional.
...
Obviously, running CFD simulations to calculate the resistance is more computationally demanding than the approximate analytical scheme explained in the previous section
