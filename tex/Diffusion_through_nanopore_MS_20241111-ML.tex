...
We identified critical values $\chi^{\text{crit}}_{\text{PC}}$ and $d_{\text{crit}}$ (polymer-colloid afinity and particle diameter, respectively), which correspond to the conditions 
at which the resistance of the brush-filled pore equals that of the bare pore, i.e., $R = R_{\text{empty}}$.
...
The dependence of insertion free energy on particle size is cubic, so for particles near a critical size $d_{\text{crit}}$, a slight change in particle size 
translates into a drastic change in permeability (resistance).
The selectivity of permeation with respect to particle size increases with the critical particle size, $d_{\text{crit}}$, which itself grows as particle adsorption strength increases (indicated by a decrease in $\chi_{\text{ads}} \sim \chi_{\text{PC}}$), eventually leading to a "gating" behavior, so that a minor change in particle size drastically change permeation rate.
This is demonstrated in Figure \ref{fig:R_vs_d}, where a step-like change in resistance is observed for particles with high affinity ($\chi_{\text{PC}}$) 
and large $d_{\text{crit}}$, with resistance switching from below that of a bare pore to a much higher level upon a minor change in particle size.
With decreasing solvent strength (increasing $\chi_{\text{PS}}$), the particle size corresponding to to gating shifts to larger values, 
as adsorption strength increases (decreased $\chi_{\text{ads}}$).
For similar reasons, larger particles with affinity close to the critical value $\chi^{\text{crit}}_{\text{PC}}$ exhibit gating behavior 
in response to particle affinity, as a slight change in $\chi_{\text{PC}}$ can shift the insertion free energy from largely negative to largely positive.
Again, a decrease in solvent strength (increase in $\chi_{\text{PS}}$) shifts $\chi^{\text{crit}}_{\text{PC}}$ to higher values, 
as poorer solvents increase adsorption strength (decrease $\chi_{\text{ads}}$).
