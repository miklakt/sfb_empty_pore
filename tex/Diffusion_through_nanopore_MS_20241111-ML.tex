...
In the present paper, we present a theory to explain the physical principles that govern the diffusive transport of colloids through mesopores with a polymer brush grafted to the inner surface of the pore.
...
We demonstrate that polymer brush mesopores are selective for particle affinity to the polymer and exhibit higher selectivity to particle size compared to a bare pore.

We explore how parameters such as solvent quality, particle size, and particle adhesion to the polymer determine flux through the pore (or resistance) and pore selectivity.
...
We identified critical values $\chi^{\text{crit}}_{\text{PC}}$ and $d_{\text{crit}}$, which correspond to the points at which the resistance of the brush-filled pore equals that of the bare pore, i.e., $R = R_{\text{empty}}$.
...
In this paper, we analyze the insertion free energy landscape and show how it can shift, influenced by solvent strength, particle affinity, or particle size, from an energy well dominated by favorable adhesive interactions to an energy barrier created by osmotic repulsion.

The research employed scaling theory \cite{Cai2011} to assess the localized slowdown of particle diffusion due to polymer entanglement within the brush.
...
The strong quantitative agreement demonstrates the accuracy of our analytical theory within the limit of particle size being sufficiently smaller than the pore radius.

Here are some key findings:

\textbf{The polymer brush slows down inert or slightly adsorbing colloid particles.}
As shown in Figures \ref{fig:R_vs_chi_PC} and \ref{fig:R_vs_d}, particles with an interaction parameter $\chi_{\text{PC}} \gtrsim -0.75$ experience higher resistance compared to a bare pore for particles of any size.
...
\textbf{Facilitated permeation} (i.e., resistance lower than that of a bare pore) \textbf{occurs for sufficiently attractive particles when a path with negative insertion free energy exists} in the free energy landscape $\forall z \exists r, \Delta F(r,z) < 0$, as shown in Figure \ref{fig:DeltaF_map} for $\chi_{\text{PC}} \le -1.00$.
...
\textbf{The permeability of the polymer brush pore is limited by the exterior region}, which sets a lower bound on the total resistance of the pore.
While this depends on boundary conditions, this phenomenon is observed for any system where the diffusing particle must travel a sufficiently long path through pure solvent via diffusion, as is the case for nucleopores [\dots].
...
In the case of very large negative insertion free energy ($\Delta F \ll 0$), approaching infinitely negative values ($\Delta F \to -\infty$), the total resistance becomes limited by the resistance of the exterior region, with $R_{\text{pore}} \to 0$ and $R_{\text{conv}} \to \frac{1}{D_0 2 \pi r_{\text{ext}}}$.
...
Since local conductivity is exponential with respect to local insertion free energy, total resistance is also exponentially connected to the insertion free energy landscape.
...
The dependence of insertion free energy on particle size is cubic, so for particles near a critical size $d_{\text{crit}}$, a slight change in particle size translates into a drastic change in permeability (resistance).
The selectivity of permeation with respect to particle size increases with $d_{\text{crit}}$, which itself increases with particle adsorption strength, represented by a decrease in $\chi_{\text{ads}} \sim \chi_{\text{PC}}$, up to a behavior recognized as "gating."
This is demonstrated in Figure \ref{fig:R_vs_d}, where a step-like change in resistance is observed for particles with high affinity ($\chi_{\text{PC}}$) and large $d_{\text{crit}}$, with resistance switching from below that of a bare pore to a much higher level upon a minor change in particle size.
With decreasing solvent strength (increasing $\chi_{\text{PS}}$), the particle size corresponding to to gating shifts to larger values, as adsorption strength increases (decreased $\chi_{\text{ads}}$).
For similar reasons, larger particles with affinity near critical value $\chi^{\text{crit}}_{\text{PC}}$ exhibit gating behavior in response to particle affinity, as a slight change in $\chi_{\text{PC}}$ can shift the insertion free energy from largely negative to largely positive.
Again, a decrease in solvent strength (increase in $\chi_{\text{PS}}$) shifts $\chi^{\text{crit}}_{\text{PC}}$ to higher values, as poorer solvents increase adsorption strength (decrease $\chi_{\text{ads}}$).
...
Interestingly, high partitioning of diffusing species within the brush does not necessarily equate to a high permeation rate.
...
In other words, while high partitioning requires negative insertion free energy ($\Delta F(r,z) < 0$), facilitated permeation requires a pathway with negative insertion free energy, such that $\forall z \exists r, \Delta F(r,z) < 0$.
