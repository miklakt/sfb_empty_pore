...
%For an empty pore the influx rate of small colloids can be estimated as:


In the realm of nucleopores, the finite reservoir is the cell nucleus, for the HeLa cells the next estimates are available:
$V_{\text{nucleus}} \approx 1200$,
$N_{\text{pores}} \approx 3000 \text{fL}$,
and the viscosity of the medium at 20$^\circ$ is $\eta_{\text{S}} = 0.00145 \text{Pa}\cdot \text{s}$.

This allow us to compare our theoretical results with available experimental data on passage rate in ref \cite{Ribbeck2001} where the influx rate of inert particles through nucleopores was examined.


As we discussed for attractive particles with low $\chi_{\text{PC}}$ we could expect to see enhanced compared to inert particles permeability, with the absolute value potentially above that of a bare pore.
...
Following procedure from ref \cite{Schmidt2015} small droplets of Nup98 FG-domains were prepared by phase separation by rapid dilution of FG-domain solution in aqueous solution. The estimated protein concentration is $\approx 200 \, \text{mg}/\text{mL}$ which translates to the volume concentration of $\phi_{\text{gel}} \approx 0.2$ assuming typical 'dry' protein density $1.25 \, \text{mg}/\text{mL}$.
...
Similarly, drops of Nup116 FG-domains were prepared with estimated protein concentration is $\approx 400 \, \text{mg}/\text{mL}$ which translates to the volume concentration of $\phi_{\text{gel}} \approx 0.4$.

When diluted, FG-domains phase separate from aqueous solution, the author reported \cite{Schmidt2015} the critical concentration for phase separation $\approx 200 \, \text{nM}$, given the concentration after dilution were about 200-fold higher and very fast process of particle assembly, we believe this was spinodal decomposition. 





% Assume a uniform insertion free energy in the pores with particles well-mixed in the pore compartments.
% The equilibration is described as three-compartment model.
% Particles will distribute to the pores according to Boltzmann equilibrium $c_{\text{pore}} = c_{\text{bulk}} e^{\Delta F / k_{\text{B}} T}$.
% The equilibration kinetics is a system two ODE
% \begin{eqnarray}
%     \begin{cases}
%         \frac{\partial c}{\partial t} =  \frac{N_{\text{pore}}}{V_{\text{pore}}} \left(c_{\text{bulk}} - \frac{c_{\text{pore}(t)}}{e^{\Delta F / k_{\text{B}} T}}\right)
%     \end{cases}
% \end{eqnarray}


% The passive flux of a mobile species into nucleus (insertion free energy $\Delta F \ge 0$) follows 
\end{multicols}
