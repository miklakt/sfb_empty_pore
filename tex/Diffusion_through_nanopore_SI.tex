...
\begin{figure*}[h]
    ...
    \caption{%
        ...
        Pore radius $r_{\text{p}}^{0} = 20$ and thickness $L_{0} = 20$ were chosen for illustrative purposes.
        \todo{RR: Can we use $2r_{\text{p}}^{0} = L_{0} = 56$ instead, for consistency with the remainder of the manuscript?}
        The membrane is illustrated in striped green.
        \todo{RR: Can we use labels a and b instead of left and right here, for enhanced clarity?}
    }
...
For a pore in an infinitely thin membrane, the iso-concentration surfaces are known to form oblate spheroids with the pore rim as their focal circle.
\todo{RR: Mikhail, please add a reference.}
...
A polymer filling within the pore modifies the local diffusion coefficient $D$ and generates a free-energy landscape, resulting in an effective position-dependent diffusion coefficient (conductivity) $\tilde{D}(r,z) = D\,e^{-\Delta F/k_B T}$, or local resistivity $\rho = \tilde{D}^{-1}$.
...
\begin{figure}[]
    ...
    \caption{
        Schematice of the numerical integration of the local conductivity/resistance on a cylindrical lattice.    
        \textbf{Left:}
        ...
        In the interior region, integration is performed over disk-shaped layers (bluel), with a selected layer highlighted in darker blue.
...
The latter data serve as a reference for the approximate analytical model.
...
The method uses lattice approximations, a (local) mean-field approximation with short-ranged interactions parameterized by Flory-Huggins interaction parameters, and employs the freely joined chain model for conformational degrees of freedom of polymer chains composed of segments that fit on the lattice sites. 
...
Each polymer chain was represented as a freely jointed chain (FJC) with $s=1, \cdots, N$ segments of length $a$, modeled as a step-weighted random walk on the lattice (Figure~\ref{fig:sf-scf_scheme}, right; red circles with black connecting lines).
...
Within the FJC approach, there are two complementary propagators used to generate all (mentioned) once integrated statistical weights:
\todo{RR: There seems to be an issue with the syntax of this sentence. Please can you check and correct.}
\begin{eqnarray}
    G(\{r,z\},s|1) =G(r,z) \langle  G(\{r,z\},s-1|1)\rangle \\
    G(\{r,z\},s|N) =G(r,z) \langle  G(\{r,z\},s+1|N)\rangle   
\end{eqnarray}
where the angular brackets are computed similarly as for the site fractions mentioned above.
...
It was checked whether these new volume fractions obey to the incompressibility limit, and the values of $\alpha(r, z)$ were adjusted if they did not.
The segement potentials where then re-evaluated, and a new guess was generated when the input- and output potentials differed.
The evaluation loop was repeated until the desired accuracy was reached, that is, when the difference between input and output potentials and errors with respect to the incompressibility constraint were less than 7 significant digits.
From the resulting SCF solution, the excess free energy was then evaluated.
