...
\section{Supplementary Methods 7. Analytical Estimation of Pore Resistance}
\begin{multicols}{2}
...
\begin{equation*}
    \begin{cases}
        z_{a} = -z_{\text{ext}},\quad z_{b} = -L/2-1,&\text{if } z < -L/2,\\[4pt]
        z_{a} = L/2,\quad z_{b} = z_{\text{ext}}-1,&\text{if } z > L/2.
    \end{cases}
\end{equation*}
...
% \begin{eqnarray}
%   \psi(\bm r)=c(\bm r)\,e^{\Delta F(\bm r)}, \label{eq:transform_1}\\
%   \tilde D(\bm r)=D(\bm r)\,e^{-\Delta F(\bm r)}, \label{eq:transform_2}
% \end{eqnarray}
% which define a modified concentration $\psi(\bm r)$ and a modified diffusion coefficient $\tilde D(\bm r)$.
...
% Once the stationary field $\psi(\bm r)$ is known, the flux density is found from the generalized Fickian equation:
% \begin{equation}
...
The finite volume element is indexed using its lower-left node ($r_-, z_-$).
...
\begin{eqnarray}
    \begin{aligned} 
        \nabla_{i,k} \psi = 
        \\
        \lambda^{z+}_{i,k} D^{z+} (\psi_{i,k} - \psi_{i,k+1}) +  \lambda^{z-}_{i,k} D^{z-} (\psi_{i,k} - \psi_{i,k-1})\\
        \lambda^{r+}_{i,k} D^{r+} (\psi_{i,k} - \psi_{i+1,k}) +  \lambda^{r-}_{i,k} D^{r-} (\psi_{i,k} - \psi_{i-1,k})
    \end{aligned}
...
\begin{eqnarray}
    \lambda^{z\pm}_{i,k} = \frac{1}{\delta z^2}\\
    \lambda^{r+}_{i,k} = \frac{2 r_i + 2 \delta r}{2 r_i + 1} \frac {1}{\delta r^2}\\
    \lambda^{r-}_{i,k} = \frac{2 r_i}{2 r_i + 1} \frac {1}{\delta r^2}
\end{eqnarray}
...
% For the nodes that fall to impermeable membrane no-flux conditions are set.
...
\begin{eqnarray}
    \textbf{Wall}_{0} [i,k] = 
    \begin{cases}
        1 \quad \text{if } r_i \ge r_{\text{p}}^0 \text{ and } z_k \ge L_0 \\
...
\begin{equation}
    r_{\text{max}} - \delta r = \sqrt{z_{\text{min}}^2 + r_{\text{p}}^2}
\end{equation}
...
\begin{equation}
  ...
  \begin{cases}
    ...
    -(2 D_m+ D^{z-}) \cfrac{1}{\delta z^2} & \text{if } k = N_z-1, \\
...
\begin{eqnarray}
    ...
    \begin{cases}
        0 &  \text{if } m-1 \in \textbf{Wall} \\
        2 D^{z-} \lambda^{z-} & \text{if }  k = N_z-1 \\
        D^{z-} \lambda^{z-} &  \text{otherwise}
    \end{cases}
    \\
    \bm{L}_{m,m \pm N_z} =
    \begin{cases}
        0 \quad \text{if } m \pm N_z \in \textbf{Wall} \text{ or }  k = N_z-1 \\
        D^{r\pm} \lambda^{r\pm}  \text{otherwise}
    \end{cases}
\end{eqnarray}


The right-hand side vector $\bm{b}$ enforces the boundary condition from the source:
\begin{eqnarray}
    \bm{b}_m = 
    \begin{cases}
        1 & \text{if } m \in \textbf{Source} \\
...
\todo{NOT FINISHED}
After solving for $\bm{\psi}$ we reshape the vector back to $\psi(r,z)$ and retrieve the physical concentration $c(r,z)=\psi\,e^{-\Delta F_(r,z)}$.

The total colloid flux is approximated on the grid by
\begin{equation}
  J_{\text{num}} \approx 2\pi\sum_{i=0}^{N_r-1}
      \tilde D_{i N_z + k_0}\,
...
The resulting pore resistance is 
\begin{eqnarray}
    R_{\text{num}} = 2 \Delta c / J_{\text{num}} + 2 R_{(|z_\text{min}-L/2|, -\infty)}
\end{eqnarray}
where $R_{(|z_\text{min}-L/2|, -\infty)}$ is reservoir resistance, as given by Eq.~\eqref{eq:r_reservoir}.
