...
for chi_PS_, result in results.groupby(by = "chi_PS"):
    ...
    ax.plot(chi_PC, R_tot2, label = chi_PS_,  color = "k", linewidth = 0,  marker = "s", mfc = "none", ms = 3)

ax.plot(chi_PC, R_thin, label = r"$R_{\text{ext}}^{0}$", color = "red", linewidth = 0.6)
ax.plot(chi_PC, R_thick, label = r"$R_{0}$", color = "red", linewidth = 0.6)
